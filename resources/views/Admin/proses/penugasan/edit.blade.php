    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <form action="{{ route('admin.penugasan.update', $penugasan->id_penugasan) }}" method="POST">
                ...
                <div class="bg-white border rounded-2xl p-5 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="text" value="{{ $penugasan->servis->kode_servis }}" readonly class="w-full border bg-gray-100 rounded-xl p-3 mt-1">
                        </div>

                        <div>
                            <label class="text-xs font-bold text-gray-500 uppercase">Pelanggan</label>
                            <input type="text" value="{{ $penugasan->servis->booking->pelanggan->nama }}" readonly class="w-full border bg-gray-100 rounded-xl p-3 mt-1">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border rounded-2xl p-5 shadow-sm">
                    ...
                    <select disabled class="w-full border bg-gray-100 rounded-xl p-3 outline-none cursor-not-allowed">
                        @foreach ($teknisi as $t)
                            <option value="{{ $t->id_user }}" {{ $penugasan->id_user == $t->id_user}}>
                                {{ $t->nama }}
                            </option>
                        @endforeach
                    </select>
                    <input type="hidden" name="id_user" value="{{ $penugasan->id_user }}">
                </div>
                ...
                <div class="bg-white border rounded-2xl p-5 shadow-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <input type="date" name="estimasi_selesai" value="{{ $penugasan->estimasi_selesai }}" class="w-full border rounded-xl p-3 focus:ring-2 focus:ring-yellow-500 outline-none">
                        </div>
                    </div>
                    ...
                    <div class="mt-5">
                        ...
                        <textarea name="catatan_teknisi" rows="3" class="w-full border rounded-xl p-3 focus:ring-2 focus:ring-yellow-500 outline-none">{{ $penugasan->catatan_teknisi }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="px-6 py-4 bg-white border-t flex justify-end gap-3">
                <a href="{{ route('admin.penugasan.index') }}" class="px-5 py-3 rounded-xl border text-gray-600 hover:bg-gray-50">Kembali</a>
                <button type="submit" class="px-6 py-3 rounded-xl bg-yellow-500 hover:bg-yellow-600 text-white font-semibold">Update Penugasan</button>
            </div>
        </form>
    </div>
</div>
