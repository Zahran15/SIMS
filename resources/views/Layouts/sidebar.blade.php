...
{{-- Inisialisasi AlpineJS: Di HP default-nya selalu true agar lebarnya penuh saat ditarik --}}
<div x-data="{ open: true }" class="flex h-full">

    {{-- 
        Perubahan Kelas Responsif:
        - HP: Lebar selalu w-[280px], melayang penuh (h-screen)
        - Desktop (md:): Lebar mengikuti variabel AlpineJS (:class open)
    --}}
    <aside class="bg-blue-600 h-screen flex flex-col shadow-2xl z-50 border-r border-white/5 transition-all duration-300 w-[280px] md:relative"
           :class="window.innerWidth >= 768 ? (open ? 'md:w-[280px]' : 'md:w-[80px]') : 'w-[280px]'">

        <header class="h-[73px] bg-white flex items-center justify-between px-4 border-b overflow-hidden shrink-0">
            ...
            {{-- Tombol Collapse internal: Hanya muncul di layar desktop (md:block), di HP kita sembunyikan --}}
            <button @click="open = !open" class="hidden md:block text-gray-500 hover:text-blue-600 p-2 rounded-xl transition-colors mx-auto">
        ...
        <div class="flex-1 overflow-y-auto px-3 py-6 custom-scroll" x-data="{ activeMenu: '{{ Request::segment(2) }}' }">
            ...
            <a href="/{{ $role }}/dashboard"
               ...
               :class="[
                ...
                <i class="fas fa-gauge-high text-lg transition-colors duration-300 shrink-0" 
            ...
            <div class="space-y-1 mb-2">
                <button @click="open ? (activeMenu = activeMenu === 'master' ? null : 'master') : open = true"
                    ...
                    :class="open ? 'justify-between' : 'md:justify-center'">
                    <span class="flex items-center gap-3 text-sm font-bold tracking-wide">
                        <span class="w-6 flex justify-center shrink-0"><i class="fas fa-database text-blue-1000 text-lg"></i></span> 
                ...
                <div x-show="activeMenu === 'master' && open" x-transition class="pl-11 space-y-2 pb-2">
                    ...
                    <a href="/admin/jasa_servis" class="block text-xs text-white hover:text-emerald-500 transition font-bold uppercase tracking-widest whitespace-nowrap">Data Jasa Servis</a>
                </div>
            </div>
            ...
            <div class="space-y-1 mb-2">
                <button @click="open ? (activeMenu = activeMenu === 'pengadaan' ? null : 'pengadaan') : open = true"
                    ...
                    :class="open ? 'justify-between' : 'md:justify-center'">
                    <span class="flex items-center gap-3 text-sm font-bold tracking-wide">
                        <span class="w-6 flex justify-center shrink-0"><i class="fas fa-truck-loading text-blue-1000 text-lg"></i></span>
            ...
            <div class="space-y-1 mb-2">
                <button @click="open ? (activeMenu = activeMenu === 'proses' ? null : 'proses') : open = true"
                    ...
                    :class="open ? 'justify-between' : 'md:justify-center'">
                    <span class="flex items-center gap-3 text-sm font-bold tracking-wide">
                        <span class="w-6 flex justify-center shrink-0"><i class="fas fa-layer-group text-blue-1000 text-lg"></i></span> 
            ...
            <div class="space-y-1 mb-2">
                <button @click="open ? (activeMenu = activeMenu === 'pengaturan' ? null : 'pengaturan') : open = true"
                    ...
                    :class="open ? 'justify-between' : 'md:justify-center'">
                    <span class="flex items-center gap-3 text-sm font-bold tracking-wide">
                        <span class="w-6 flex justify-center shrink-0"><i class="fas fa-cogs text-blue-1000 text-lg"></i></span> 
            ...
            <div x-show="open" class="text-[10px] uppercase text-white mt-8 mb-3 px-4 font-black tracking-[0.2em] whitespace-nowrap">Executive Owner</div>
                ...
                <div class="space-y-1 mb-2">
                    <button @click="open ? (activeMenu = activeMenu === 'monitoring' ? null : 'monitoring') : open = true"
                        ...
                        :class="open ? 'justify-between' : 'md:justify-center'">
                        <span class="flex items-center gap-3 text-sm font-bold tracking-wide">
                            <span class="w-6 flex justify-center shrink-0"><i class="fas fa-eye text-blue-1000 text-lg"></i></span> 
                ...
                <div class="space-y-1 mb-2">
                    <button @click="open ? (activeMenu = activeMenu === 'laporan' ? null : 'laporan') : open = true"
                        ...
                        :class="open ? 'justify-between' : 'md:justify-center'">
                        <span class="flex items-center gap-3 text-sm font-bold tracking-wide">
                            <span class="w-6 flex justify-center shrink-0"><i class="fas fa-file-contract text-blue-1000 text-lg"></i></span> 
            ...
            @if($role == 'teknisi')
                ...
                <div class="space-y-1 mb-2">
                    <button @click="open ? (activeMenu = activeMenu === 'proses' ? null : 'proses') : open = true"
                        ...
                        :class="open ? 'justify-between' : 'md:justify-center'">
                        <span class="flex items-center gap-3 text-sm font-bold tracking-wide">
                            <span class="w-6 flex justify-center shrink-0"><i class="fas fa-tools text-blue-1000 text-lg"></i></span> 
                            <span x-show="open" class="whitespace-nowrap">OPERASIONAL</span>
                        </span>
                        <i x-show="open" class="fas fa-chevron-right text-[10px] transition-transform duration-300" :class="{ 'rotate-90 text-blue-1000': activeMenu === 'proses' }"></i>
                    </button>
                    <div x-show="activeMenu === 'proses' && open" x-transition class="pl-11 space-y-2 pb-2">
                        <a href="/teknisi/servis_kerja" class="block text-xs text-white hover:text-emerald-500 transition font-bold uppercase tracking-widest whitespace-nowrap">Tugas Servis</a>
            ...
            @if($role == 'pelanggan')
                ...
                <div class="space-y-1 mb-2">
                    <button @click="open ? (activeMenu = activeMenu === 'layanan' ? null : 'layanan') : open = true"
                        ...
                        :class="open ? 'justify-between' : 'md:justify-center'">
                        <span class="flex items-center gap-3 text-sm font-bold tracking-wide">
                            <span class="w-6 flex justify-center shrink-0"><i class="fas fa-user-gear text-blue-1000 text-lg"></i></span> 
                    ...
                    <div x-show="activeMenu === 'layanan' && open" x-transition class="pl-11 space-y-2 pb-2">
                        ...
                        <a href="/pelanggan/status_servis" class="block text-xs text-white hover:text-emerald-500 transition font-bold uppercase tracking-widest whitespace-nowrap">Cek Status Servis</a>
        ...
        <div class="p-2 mt-auto shrink-0">
            {{-- 
                Modifikasi Kotak Putih:
                Menggunakan `md:p-3` dan `:class` agar ketika di-collapse (desktop), 
                padding menghilang sehingga tombol merahnya tidak terhimpit.
            --}}
            <div class="bg-white rounded-2xl p-3 mb-4 border border-white/5 transition-all"
                 :class="open ? 'p-3' : 'md:p-1'">
                <a href="/logout"
                   @click.prevent="
                        Swal.fire({
                            title: 'Apakah Anda yakin?',
                            text: 'Anda akan keluar dari sesi sistem ini.',
                            icon: 'warning',
                   ...
                   :title="!open ? 'Keluar Sistem' : ''">
                    <i class="fas fa-power-off text-base shrink-0"></i> 
