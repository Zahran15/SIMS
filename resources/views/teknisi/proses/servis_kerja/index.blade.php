...
@section('content')
        ...
        <form action="{{ route('teknisi.servis_kerja.index') }}" method="GET">
            <div class="flex flex-col md:flex-row gap-4 md:items-end">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('teknisi.servis_kerja.index') }}" class="px-4 py-2 bg-gray-500 text-white rounded-lg hover:bg-gray-600 text-sm transition font-medium">Reset</a>
                </div>
            </div>
        </form>
    </div>
    ...
    @if(request('kode_servis') || request('nama_pelanggan') || request('status_penugasan') || request('prioritas'))
            ...
            <table class="w-full data-table">
                ...
                <tbody class="text-gray-700 text-sm">
                    ...
                    <tr class="border-b hover:bg-gray-50 transition-colors">
                        ...
                        {{-- PERBAIKAN: KOLOM PRIORITAS (MENDUKUNG NULL & WARNA BAGUS) --}}
                        <td class="px-5 py-4 text-center whitespace-nowrap">
                            @if($p->prioritas == 'berat')
                                <span class="inline-block px-3 py-1 bg-red-100 text-red-700 rounded-full text-xs font-bold">Berat</span>
                            @elseif($p->prioritas == 'sedang')
                                <span class="inline-block px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full text-xs font-bold">Sedang</span>
                            @elseif($p->prioritas == 'ringan')
                                <span class="inline-block px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold">Ringan</span>
                            @else
                                <span class="text-gray-400 font-bold">-</span> {{-- Muncul ini jika prioritas NULL --}}
                            @endif
                        </td>

                        <td class="px-5 py-4 text-center font-medium text-gray-800">
                            @if($p->estimasi_selesai)
                                {{ \Carbon\Carbon::parse($p->estimasi_selesai)->format('d M Y') }}
                            @else
                                -
                            @endif
                        </td>

                        {{-- PERBAIKAN: WARNA STATUS MENGIKUTI PROGRES MASING-MASING STATUS --}}
                        <td class="px-5 py-4 text-center whitespace-nowrap">
                            @php
                                $statusClasses = [
                                ...
                                $currentClass = $statusClasses[$p->status_penugasan] ?? 'bg-gray-100 text-gray-600';
                            @endphp
                            <span class="inline-block px-3 py-1 rounded-full text-xs font-bold {{ $currentClass }}">
                                {{ strtoupper($p->status_penugasan) }}
                            </span>
                        </td>
                        ...
                        <td class="px-5 py-4 text-center">
                            <div class="flex justify-center items-center gap-2">
                                ...
                                {{-- KERJAKAN/EDIT --}}
                                @if($p->status_penugasan !== 'selesai' && $p->status_penugasan !== 'gagal')
