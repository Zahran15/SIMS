...
@section('content')
    ...
    <div class="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b pb-3">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Detail Status Servis</h2>
    ...
    <div class="space-y-4">
            ...
            <div class="p-4 grid grid-cols-2 gap-4 text-xs">
                <div>
                    ...
                    <span class="px-2 py-0.5 rounded font-bold border uppercase inline-block
                        ...
                        {{ $servis->status_servis == 'sudah diambil' ? 'bg-green-100 text-green-800 border-gray-200' : '' }}
                ...
                <div>
                    ...
                    {{-- FIX: Menggunakan @class agar isi teks muncul simetris & terhindar dari bug blank text --}}
                    <span @class([
                        'px-2 py-0.5 rounded font-bold border uppercase inline-block',
                        'bg-gray-100 text-gray-800 border-gray-200' => !$servis || $servis->penugasan->status_penugasan == 'belum dikerjakan',
                        'bg-blue-100 text-blue-800 border-blue-200' => $servis && $servis->penugasan->status_penugasan == 'sedang dikerjakan',
                        'bg-orange-100 text-orange-800 border-orange-200' => $servis && $servis->penugasan->status_penugasan == 'menunggu sparepart',
                        'bg-green-100 text-green-800 border-green-200' => $servis && $servis->penugasan->status_penugasan == 'selesai',
                        'bg-red-100 text-red-800 border-red-200' => $servis && $servis->penugasan->status_penugasan == 'gagal',
                    ])>
                        {{ $servis ? $servis->penugasan->status_penugasan : 'Belum Ditugaskan' }}
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded border border-gray-200 shadow-sm overflow-hidden">
            ...
            {{-- AKUMULASI BIAYA BAWAH DENGAN POTONGAN DP --}}
            <div class="bg-gray-50 border-t border-gray-200 p-3 flex justify-end">
                @php
                    $subtotal_kotor = $servis->detailJasa->sum('subtotal') + $servis->detailSparepart->sum('subtotal');
                    $nominal_dp = $servis->booking->nominal_dp ?? 50000;
                @endphp
                <div class="w-full sm:w-72 space-y-1.5 text-xs">
                    <div class="flex justify-between text-gray-500">
                        <span>Total Item Perbaikan:</span>
                        <span class="font-medium text-gray-800">Rp {{ number_format($subtotal_kotor, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between text-red-600 bg-red-50 px-1.5 py-0.5 rounded font-medium">
                        <span>Potongan DP (Sudah Dibayar):</span>
                    ...
                    <div class="flex justify-between text-sm font-bold text-gray-900 pt-1.5 border-t border-gray-200">
                        ...
                        <strong class="text-lg text-indigo-600 font-black">
                            Rp {{ number_format($servis->total_biaya - $nominal_dp, 0, ',', '.') }}
                        </strong>
                    </div>
                </div>
            </div>
        ...
        @elseif($servis->status_servis == 'sudah diambil')
            {{-- TAMBAHAN: Alert Khusus Ketika Status Servis Sudah Diambil --}}
            <div class="p-3 bg-gray-50 border border-gray-200 rounded text-xs text-gray-700 flex items-start gap-2 shadow-sm">
                <div>
                    <strong class="block mb-0.5 text-gray-800">Unit Perangkat Sudah Diambil</strong>
                    <span>Transaksi ini telah selesai. Unit perangkat beserta nota fisik telah diserahkan kembali kepada Anda. Terima kasih telah mempercayakan perbaikan perangkat Anda di Seven Komputer!</span>
                </div>
            </div>
        @endif
    </div>
